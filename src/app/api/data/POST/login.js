...
module.exports = async (req, res) => {
  ...
  if (isEmpty) return res.send({ message: "Vui lòng nhập đầy đủ thông tin" });
  ...
  try {
    ...
    if (!userList[0].refesh_token) {
      ...
      const updateToken = await conn.update(
        "UPDATE taikhoan SET refesh_token=? WHERE user_name = ?",
        [refesh_token, username]
      );
    ...
    return res.send({
      ...
      vaitro_id: userList[0].vaitro_id,
    });
  } catch (error) {
    console.log(error);
  }
