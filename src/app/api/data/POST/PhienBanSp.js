...
module.exports = async (req, res) => {
  ...
  switch (true && type != "delete") {
    case !Validator.isNumber(ram):
      return res.json({
        message: "Ram không được trống, phải là số và lớn hơn 0",
        success: false,
      });
    case !Validator.isNumber(dung_luong):
      return res.json({
        message: "Dung lượng không được trống, phải là số và lớn hơn 0",
        success: false,
      });
  }
  ...
  try {
    ...
    if (type === "insert") {
      ...
      if (insertSanPham.status !== 200) {
        return res.json({
          message: "Thêm phiên bản sản phẩm không thành công",
          success: false,
        });
      }
    ...
    if (type === "update") {
      ...
      if (updateSanPham.status !== 200) {
        return res.json({
          message: "Sửa phiên bản sản phẩm không thành công",
          success: false,
        });
      }

      return res.json({
        message: "Sửa sản phiên bản sản phẩm thành công",
        success: true,
      });
    }
    if (type === "delete") {
      ...
      if (updateTrangThai.status !== 200) {
        return res.json({
          message: "Thay đổi trạng thái phiên bản sản phẩm không thành công",
          success: false,
        });
      }

      return res.json({
        message: "Thay đổi trạng thái phiên bản sản phẩm thành công",
        success: true,
      });
    }
  } catch (error) {
