  ...
  try {
    ...
    const productByIdList = await conn.select(
      `
        ...
        WHERE sp.id_sanpham = ? AND pbsp.trangthai = 1
    ...
    for (const item of productByIdList) {
      ...
      const soLuongQuery = await conn.select(
        `SELECT COUNT(imei) as so_luong FROM 
            ...
            (cthoadon.imei IS NULL AND ctgiohang.imei IS NULL))) c WHERE stt = 1;`,
        [item.id_phienban]
      );
