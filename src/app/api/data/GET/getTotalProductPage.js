...
module.exports = async (req, res) => {
  ...
  parameterList.push(
    Number.parseInt(lowestPrice / Constant.profit),
    Number.parseInt(highestPrice / Constant.profit)
  );
  ...
  try {
    ...
    const [productList] = await conn.select(
      `SELECT COUNT(id_sanpham) as so_luong FROM(
      SELECT ROW_NUMBER() OVER(PARTITION BY sanpham.id_sanpham) c,sanpham.id_sanpham,sanpham.ten_sanpham,sanpham.heDieuHanh, phienbansanpham.ram,phienbansanpham.dung_luong, thuonghieu.ten_thuonghieu, ctphieunhap.gia
          ...
          WHERE phienbansanpham.id_sanpham AND sanpham.trangthai=1
    ...
    res.send({
      success: true,
      data: Math.ceil(productList.so_luong / itemPerPage),
    });
    conn.closeConnect();
  } catch (error) {
    res.send(error);
