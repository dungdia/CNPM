  ...
  for (const Method of list) {
    ...
    if (type == "GET") {
      if (authRoute.includes(parent)) {
        ...
        // return
      } else app.get(`/api/${parent}/${apiName}`, apiCallback);
    }
    if (type == "POST") {
      if (authRoute.includes(parent)) {
        app.post(`/api/${parent}/${apiName}`, authMiddleWare, apiCallback);
        // return
      } else app.post(`/api/${parent}/${apiName}`, apiCallback);
    }
    if (type == "PUT") {
      if (authRoute.includes(parent)) {
        app.put(`/api/${parent}/${apiName}`, authMiddleWare, apiCallback);
        // return
      } else app.put(`/api/${parent}/${apiName}`, apiCallback);
    }
    if (type == "DELETE") {
      if (authRoute.includes(parent)) {
        app.delete(`/api/${parent}/${apiName}`, authMiddleWare, apiCallback);
        // return
      } else app.delete(`/api/${parent}/${apiName}`, apiCallback);
    }
  }
}
