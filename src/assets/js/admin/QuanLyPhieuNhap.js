...
async function renderReceiptDetail(data,type){
    ...
    async function reloadDetailTable(id_sanpham){
        ...
        const jsonData = await fetchJsonData(`getAllChiTietSP?id_sanpham=${id_sanpham}`)
    ...
    const chooseProductDetail = document.getElementById("btn-choose-product-detail")
    ...
    popUpBody.innerHTML = `
        ...
        <div class="form-floating mb-3">
            <select 
                ...
                >
                ${nhacungcap.map((item)=>`<option ${type !== "add" ? (item.ten_nhacungcap === data.ten_nhacungcap ? "selected":"") : ""} value="${item.id_nhacungcap}">${item.ten_nhacungcap}</option>`).join("")}
            </select>
