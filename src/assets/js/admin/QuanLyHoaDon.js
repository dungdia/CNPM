...
async function renderOrderInfo(data) {
  ...
  const [orderDetail] = await fetchJsonData("getOneHoaDon", "GET", {
  ...
  popUpBody.innerHTML = `
        <div class="container mt-1">
            ...
            <div class="card">
                <div class="card-body">
                    <h5 style="margin-bottom: 0.5rem; font-size: 1.4rem">Hóa đơn: <span class="text-muted">${
                      orderDetail["id_hoa_don"]
                    }</span></h5>
                    <h6 style="margin-bottom: 0.5rem; font-size: 1.4rem">Khách hàng: <span class="text-muted">${
                      orderDetail["ho_ten"]
                    }</span></h6>
                    <h6 style="margin-bottom: 0.5rem; font-size: 1.4rem">Địa chỉ: <span class="text-muted">${
                      orderDetail["dia_chi"]
                    }</span></h6>
                    <h6 style="margin-bottom: 0.5rem; font-size: 1.4rem">Số điện thoại: <span class="text-muted">${
                      orderDetail["so_dien_thoai"]
                    }</span></h6>
                    <h6 style="margin-bottom: 0.5rem; font-size: 1.4rem">Ghi chú: <span class="text-muted">${
                      !orderDetail["ghi_chu"]
                        ? `Không có ghi chú`
                        : `${orderDetail["dia_chi"]}`
                    }</span></h6>
        ...
        <tfoot>
            ...
            <tr>
                <th colspan="4" style="font-weight: bold; padding-right: 110px; text-align: end">
                    ${VND.format(
                      orderDetail["chi_tiet_sp"].reduce(
                        (total, item) => total + item["gia_ban"],
                        0
                      )
                    )}
                </th>
            ...
            <tr>
                <th colspan="4">
                    <select class="form-select" id="orderStatusSelect" ${
                      orderDetail["trang_thai_hoadon"] === 4 ||
                      orderDetail["trang_thai_hoadon"] === 5
                        ? "disabled"
                        : ""
                    }>
  ...
  if (orderDetail["trang_thai_hoadon"] === 1) {
    orderStatusSelect.innerHTML = `${orderStatus
      .map(
        (item) => `
