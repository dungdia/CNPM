...
async function renderProductInfo(data, type) {
    ...
    if (data) {
        [item] = await fetchJsonData("getOneChiTietSP", "GET", { id_phienban: data.id_phienban });
    }
    ...
    productSelect.innerHTML = productList.map(item => `<option value="${item.id_sanpham}">${item.ten_sanpham}</option>`).join("");
    warrantySelect.innerHTML = warrantyList.map(item => `<option value="${item.id_baohanh}">${item.sothang} tháng</option>`).join("");
    if (type === "edit") {
...
async function showLock(data) {
    ...
    popUpBody.textContent = `Bạn có muốn ${(data.trangthai === "0" ? "khóa" : "mở khóa")} phiên bản ${data.ten_sanpham}?`;
    ...
    popUpSaveBtn.onclick = async () => {
        ...
        popUpBody.textContent = `${data.trangthai === 0 ? "khóa" : "mở khóa"} ${data.ten_sanpham} thành công`;
